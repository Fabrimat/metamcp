...
10. On application failure after migration 0020, stop all app writers and restore the complete pre-deploy PostgreSQL dump, including schema, data, and migration journal. Run `deploy/oauth-rollback-guard.sql` with `ON_ERROR_STOP=1`; only after it passes, point the override to the recorded rollback image and start `app`. App-only rollback is forbidden. The restore loses post-backup writes; follow `deploy/oauth-rollback.md` and retain a forensic dump of the failed deployment first.
